...
```{r results=FALSE, message=FALSE, echo=FALSE}
  ...
  required_packages <- c("xgboost", "data.table", "stringr", "SHAPforxgboost", "ggplot2", "gridExtra", "wordcloud", "tm")
...
data_all[, 'Relative place population density' := relative_value(city_density), by="keyword_strings"]
...
* Spearman's rank correlation:
  + In the plot below, showing the distribution of correlations calculated separately for each search   + Overall correlation is about 0.6, showing fairly good fit between observations and predictions
...
* SHapley Additive exPlanations (SHAP):
  + First plot shows average value of each features contribution over the whole data set and can be interpreted as a overall importance for predictions
  + Second plot shows the direction of the impact given feature's value. E.g. if the attribute "Has city in title" got high value (high="Yes, low="No"), it's SHAP value is also positive; meaning it will increase the predicted ranking score, where as low value doesn't change much of anything (SHAP close to zero). 
...
shap_values_plot <- shap.plot.summary.wrap2(shap_values$shap_score, X_train, dilute=10)
...
if(save == T){ 
  ggsave(here::here("plots", "titles_wordcloud.pdf"), titles_wordcloud_plot, 
         width = 12.5, height = 8, device = cairo_pdf) 
  ggsave(here::here("plots", "descriptions_wordcloud.pdf"), descriptions_wordcloud_plot, 
         width = 12.5, height = 8, device = cairo_pdf) 
...
plot_count_title <- ggplot(data_vis, aes(position, `#Characters in title`)) +
  ...
  ylab("#Characters") +
  xlab("Position") 
plot_count_description <- ggplot(data_vis, aes(position, (`#Characters in description`))) +
  stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE) +
  scale_fill_distiller(palette=4, direction=1) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  ...
  ylab("#Characters") +
  xlab("Position") 
title_description_counts_plot <- grid.arrange(plot_count_title, plot_count_description, nrow=1, ncol=2, top="#Characters vs position")
...
review_position_plot <- ggplot(data_vis, aes(position, `Relative #reviews`)) +
  stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE) +
  scale_fill_distiller(palette=4, direction=1) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
...
rating_position_plot <- ggplot(data_vis[!is.na(`Review rating`)], aes(position, `Review rating`)) +
  stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE) +
  scale_fill_distiller(palette=4, direction=1) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
