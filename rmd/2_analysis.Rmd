...
```{r results=FALSE, message=FALSE, echo=FALSE}
  ...
  required_packages <- c("xgboost", "data.table", "stringr", "SHAPforxgboost", "ggplot2", "gridExtra")
...
data_all[, target := (rank(-position, ties.method="first") - 1) / (.N-1), by=keyword_strings]
...
#gather basic info
type_data <- data_all[google_ad =="No" & type_category != "", c("keyword", "type_category", "target"), with=F]
type_data[type_category=="", type_category := "NA"]
type_counts <- as.data.frame(sort(table(type_data$type_category), decreasing=T), stringsAsFactors = F)
names(type_counts)[1] <- "category"
type_unique_data <- data_all[, list(N = length(unique(type_category))), by="keyword_strings"]

#table
type_basic <- data.table("Description" = character(), "Value" = character())
type_basic <- rbind(type_basic, list('Total unique categories', length(unique(data_all$type_category)) - 1))
type_basic <- rbind(type_basic, list('Missing type category', paste0(round(100*sum(data_all$type_category=="")/nrow(data_all), 2), "%")))
type_basic <- rbind(type_basic, list('Categories with more than >=10 results', sum(type_counts$Freq>=10)))
type_basic <- rbind(type_basic, list('Categories with more than >=100 results', sum(type_counts$Freq>=100)))
type_basic <- rbind(type_basic, list('Categories with more than >=1000 results', sum(type_counts$Freq>=1000)))
type_basic <- rbind(type_basic, list('Median unique categories in one search', median(type_unique_data$N)))
type_basic <- rbind(type_basic, list('Min unique categories in one search', min(type_unique_data$N)))
type_basic <- rbind(type_basic, list('Max unique categories in one search', max(type_unique_data$N)))

knitr::kable(type_basic, caption = "Basic information about type categories")


#plots
freq_plot <- ggplot(data=type_counts[1:10,], aes(x=reorder(category, Freq), y=Freq)) +
...
ps <- list()
for(i in 1:6) {
  ps[[i]] <- ggplot(type_data[type_category == type_counts$category[i]], aes(x=-target)) +
...
grid.arrange(ps[[1]], ps[[2]], ps[[3]], ps[[4]], ps[[5]], ps[[6]], nrow=2, ncol=3, top = "Distribution of search positions for 6 most common type categories")
```    
