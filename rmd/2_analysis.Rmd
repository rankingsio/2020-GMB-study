...
caption <- "Graphic: FrontPage Data"
...
if(save == T){ 
  ...
  # interactive chart pushed to plotly chart studio
  g <- plotly(feature_importance_plot, .1)
...
type_freq_plot <- 
  ...
  geom_text(aes(label = glue::glue("{format(Freq, big.mark = ",")} ({perc}%)")), 
            ...
            lineheight = .9, nudge_y = 500, hjust = 0) +
  ylab("Count") +
  xlab("Category") + 
...
for(i in 1:6) {
  ...
  scale_y_continuous(expand = c(0, 0), breaks = seq(0, 1250, by = 250), limits = c(0, 1400)) +
...
type_position_plot <- 
  ...
  plot_layout(nrow = 2) +
  plot_annotation(title = "Distribution of search positions for the six most common type categories")
...
if(save == T){ 
  ...
  ggsave(here::here("plots", "type_position.pdf"), type_position_plot, 
         width = 12.5, height = 8, device = cairo_pdf) 
  ggsave(here::here("plots", "type_position_scaled.pdf"), type_position_plot, 
         width = 12.5, height = 8, device = cairo_pdf) 
  
  ## !! Plotly doesnt work with multipanel plots!
  # interactive chart pushed to plotly chart studio
  g <- plotly(type_freq_plot, .15)
  api_create(g, filename = "type_freq_plot")
  g <- plotly(type_position_plotly, .2)
  api_create(g, filename = "type_position_plot")
  g <- plotly(type_position_plotly_scaled, .2)
...
desc_wordcloud <-
  ...
  ggplot(aes(label = word, size = freq, color = freq)) +
    ...
    scale_color_gradient(low = "grey85", high = cols[1]) +
    scale_size_area(max_size = 10) +
    labs(title = "\n100 most common words in the description") +
    theme_minimal() +
...
title_wordcloud <- 
  ...
  ggplot(aes(label = word, size = freq, color = freq)) +
    ...
    labs(title = "\n100 most common words in the title") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = .5, family = "Lora", face = "bold"))
...
description_missing_plot_facet <- 
  data_vis %>% 
  mutate(cat = if_else(description == "", "Yes", "No"),
...
description_car_personal_plot <-
  data_vis %>% 
  mutate(cat = if_else(`Has car accident or personal injury in description` == 1, "Yes", "No"),
         cat = factor(cat, levels = c("Yes", "No"))) %>% 
  ggplot(aes(x=position)) +
  geom_bar(fill=cols[1], width = .8) +
  scale_x_continuous(breaks = c(1, 5, 10, 15, 20), expand = c(.02, .02)) +
  ...
  ylab("Count") +
  xlab("Position") + 
  labs(title = "Has \"car accident\" or \"personal injury\" in description?") +
  facet_wrap(~cat)
...
title_car_personal_plot <-
  data_vis %>% 
  mutate(cat = if_else(`Has car accident or personal injury in title` == 1, "Yes", "No"),
         cat = factor(cat, levels = c("Yes", "No"))) %>% 
  ggplot(aes(x=position)) +
  geom_bar(fill=cols[1], width = .8) +
  scale_x_continuous(breaks = c(1, 5, 10, 15, 20), expand = c(.02, .02)) +
  ...
  ylab("Count") +
  xlab("Position") + 
  labs(title = "Has \"car accident\" or \"personal injury\" in title?") +
  facet_wrap(~cat)
...
description_lawyer_attorney_plot <-
  data_vis %>% 
  mutate(cat = if_else(`Has lawyer or attorney in description` == 1, "Yes", "No"),
         cat = factor(cat, levels = c("Yes", "No"))) %>% 
  ggplot(aes(x=position)) +
  geom_bar(fill=cols[1], width = .8) +
  scale_x_continuous(breaks = c(1, 5, 10, 15, 20), expand = c(.02, .02)) +
  ...
  ylab("Count") +
  xlab("Position") + 
  labs(title = "Has \"lawyer\" or \"attorney\" in description?") +
  facet_wrap(~cat)
...
title_lawyer_attorney_plot <-
  data_vis %>% 
  mutate(cat = if_else(`Has lawyer or attorney in title` == 1, "Yes", "No"),
         cat = factor(cat, levels = c("Yes", "No"))) %>% 
  ggplot(aes(x=position)) +
  geom_bar(fill=cols[1], width = .8) +
  scale_x_continuous(breaks = c(1, 5, 10, 15, 20), expand = c(.02, .02)) +
  ...
  ylab("Count") +
  xlab("Position") + 
  labs(title = "Has \"lawyer\" or \"attorney\" in title?") +
  facet_wrap(~cat)
...
description_city_plot <-
  data_vis %>% 
  mutate(cat = if_else(`Has same city listed as in search query` == 1, "Yes", "No"),
         cat = factor(cat, levels = c("Yes", "No"))) %>% 
  ggplot(aes(x=position)) +
  geom_bar(fill=cols[1], width = .8) +
  scale_x_continuous(breaks = c(1, 5, 10, 15, 20), expand = c(.02, .02)) +
  ...
  ylab("Count") +
  xlab("Position") + 
  labs(title = "Is city name in description?") +
  theme() +
  facet_wrap(~cat)
...
title_city_plot <-
  data_vis %>% 
  mutate(cat = if_else(`Has city in title` == 1, "Yes", "No"),
         cat = factor(cat, levels = c("Yes", "No"))) %>% 
  ggplot(aes(x=position)) +
  geom_bar(fill=cols[1], width = .8) +
  scale_x_continuous(breaks = c(1, 5, 10, 15, 20), expand = c(.02, .02)) +
  ...
  ylab("Count") +
  xlab("Position") + 
  labs(title = "Is city name in title?") +
  facet_wrap(~cat)
...
if(save == T){ 
  ...
  # interactive chart pushed to plotly chart studio
  g <- plotly(plot_count_title + ggtitle("Non-empty title length boxplots"), .15)
  api_create(g, filename = "plot_count_title")
  g <- plotly(plot_count_description + ggtitle("Non-empty description length boxplots"), .15)
  api_create(g, filename = "plot_count_description")
  g <- plotly(description_missing_plot_facet, .1)
  api_create(g, filename = "description_missing_plot_facet")
  g <- plotly(description_car_personal_plot, .1)
  api_create(g, filename = "description_car_personal_plot")
  g <- plotly(title_car_personal_plot, .1)
  api_create(g, filename = "title_car_personal_plot")
  g <- plotly(description_lawyer_attorney_plot, .1)
  api_create(g, filename = "description_lawyer_attorney_plot")
  g <- plotly(title_lawyer_attorney_plot, .1)
  api_create(g, filename = "title_lawyer_attorney_plot")
  g <- plotly(description_city_plot, .1)
  api_create(g, filename = "description_city_plot")
  g <- plotly(title_city_plot, .1)
...
rating_distribution_plot <- 
  ...
  scale_y_continuous(labels = scales::comma_format(), expand = c(0, 0), limits = c(0, 1200000))
plot(rating_distribution_plot)

review_position_plot <- ggplot(data_vis, aes(y=`Relative # of reviews`, x=as.factor(position))) +
  geom_boxplot(color=cols[1], fill="grey85") +
  ylab("Relative # of reviews") +
  xlab("Position") +
  labs(title = "Relative # of reviews boxplot")
...
if(save == T){ 
  ...
  # interactive chart pushed to plotly chart studio
  g <- plotly(rating_distribution_plot, .15)
  api_create(g, filename = "rating_distribution_plot")
  g <- plotly(review_position_plot, .15)
  api_create(g, filename = "review_position_plot")
  g <- plotly(rating_position_plot, .15)
...
if(save == T){ 
  ...
  # interactive chart pushed to plotly chart studio
  g <- plotly(ref_domains_dofollow_plot, .15)
  api_create(g, filenmae = "ref_domains_dofollow_plot")
  g <- plotly(total_traffic_plot, .15)
  api_create(g, filename = "total_traffic_plot")
  g <- plotly(ahrefs_rank_plot, .15)
  api_create(g, filename = "ahrefs_rank_plot")
  g <- plotly(domain_rating_plot, .15)
...
number_of_photos_plot <- ggplot(data_vis, aes(y=`Relative # of photos`, x=as.factor(position))) +
  geom_boxplot(color=cols[1], fill="grey85") +
  ylab("Relative # of photos") +
  xlab("Position") +
  labs(title = "Relative # of photos boxplot")
...
provides_updates_plot_facet <- 
  ...
  xlab("Position") + 
  scale_y_continuous(expand = c(0,0)) +
  labs(title = "Provides updates on Google") + 
  facet_wrap(~cat)
...
if(save == T){ 
  ...
  # interactive chart pushed to plotly chart studio
  g <- plotly(number_of_photos_plot, .15)
  api_create(g, filename = "number_of_photos_plot")
  g <- plotly(provides_updates_plot_facet, .1)
