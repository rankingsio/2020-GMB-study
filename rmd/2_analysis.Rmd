...
required_packages <- c("xgboost", "data.table", "stringr", "SHAPforxgboost", "gridExtra", "wordcloud", "tm", "grDevices", "here", "pdftools", "plotly", "patchwork", "tidyverse")
...
theme_update(plot.background = element_rect(color = "white", 
             ...
             panel.grid.major.y = element_line(color = "grey70", linetype = "dotted", size = .25),
...
type_freq_plot <- ggplot(data=type_counts[1:keep_top_N,], aes(x=reorder(category, Freq), y=Freq)) +
  ...
  geom_text(aes(label = format(Freq, big.mark = ",")), family = "Alegreya", fontface = "bold", color = "grey45", nudge_y = 500, hjust = 0) +
  ylab("Count") +
  xlab("Category") + 
...
type_position_plot <- 
  ...
  plot_layout(nrow = 2) +
  plot_annotation(title = "Distribution of search positions for 6 most common type categories")
...
rating_distribution_plot <- ggplot(all_reviews, aes(x=all_reviews_rating)) +
  geom_bar(color="white", fill=cols[1]) +
  ylab("Count") +
  xlab("Rating") + 
